...
def search_knowledge(query: str, limit: str = "8") -> str:
    """主動查詢知識庫（brain_knowledge + Chroma）。"""
    q = (query or "").strip()
...
- skill_dispatch: 技能調度（僅用清單內技能；參數不足回傳 error；嚴禁自行複雜計算）→ TOOL: skill_dispatch([\"{\\\"call\\\": \\\"survey_check\\\", \\\"params\\\": {\\\"data\\\": [0.002, 0.006, 0.001]}}\"])
  技能清單：survey_check 檢核測量數據誤差 params.data；cost_calc 計算營建材料成本 params.items；file_archive 本地檔案自動歸檔 params.path。
...
def _run_skill_dispatch(json_str: str) -> str:
    """執行技能調度。僅使用清單內技能；參數不足回傳 {\"error\": \"缺失參數: [參數名]\"}；嚴禁自行複雜計算。"""
    try:
        from skill_dispatch import dispatch
        payload = json.loads(json_str) if isinstance(json_str, str) else json_str
        out = dispatch(payload)
        return json.dumps(out, ensure_ascii=False)
    except json.JSONDecodeError as e:
        return json.dumps({"error": f"JSON 格式錯誤: {e}"}, ensure_ascii=False)
    except Exception as e:
        return json.dumps({"error": str(e)}, ensure_ascii=False)


TOOL_MAP["skill_dispatch"] = lambda args: _run_skill_dispatch(args[0]) if args else json.dumps({"error": "缺失參數: 請傳入 JSON 字串（含 call 與 params）"}, ensure_ascii=False)
