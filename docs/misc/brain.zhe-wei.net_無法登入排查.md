...
- **原因一**：cloudflared 沒在跑或未連上 Cloudflare（Token 錯、程序掛掉）。
- **原因二（常見）**：**Tunnel 用 Docker 跑時，Public Hostname 的 Service URL 設成 `localhost:8000`**。在容器內 `localhost` 是隧道自己，連不到 brain_server，Cloudflare 就會報 1033。

**處理：**

1. **若用 Docker Compose（brain_server + tunnel 都在 Docker）**
   - 到 **Cloudflare Zero Trust** → **Networks** → **Tunnels** → 選你的隧道 → **Configure** → **Public Hostname**。
   - 該筆 **Subdomain** = `brain`、**Domain** = `zhe-wei.net` 的 **Service / URL** 必須是：
     - **`http://brain_server:8000`**（Docker 內網服務名 + 容器內埠），**不要**填 `localhost:8000` 或 `localhost:8002`。
   - 儲存後等約 30 秒，再開 https://brain.zhe-wei.net/health 或 /login 測試。

2. **確認 cloudflared 有在跑**
   - Docker：`docker compose ps` 看 `tunnel` 為 **Up**；`docker compose logs tunnel --tail 20` 應有 `Registered tunnel connection`。
   - 本機：確認執行 `cloudflared tunnel run --token <TOKEN>` 的程序還在、無報錯。

3. **確認 brain_server 有回應**
   - Docker 時：本機開 `http://localhost:8002/health` 有 JSON（埠 8002 對應容器內 8000）。
   - 本機跑 brain 時：`http://localhost:8000/health` 有 JSON。
...
- Zero Trust → **Networks → Tunnels** → 選你的隧道（如 **Zhewei_Brain_Home**）→ **Configure** → **Public Hostname**。
- 確認有一筆：**Subdomain** = `brain`，**Domain** = `zhe-wei.net`。
- **Service / URL 依運行方式**：
  - **Docker Compose（brain_server 與 tunnel 都在 Docker）**：URL = **`http://brain_server:8000`**（必填，不可用 localhost）。
  - **本機跑 brain、Docker 只跑 tunnel**：URL = **`http://host.docker.internal:8002`**（Windows/Mac 用 host.docker.internal 指本機）。
  - **本機跑 brain、本機跑 cloudflared**：URL = `http://localhost:8000`。
- 若 URL 設錯（例如 Docker 卻填 localhost），會出現 **Error 1033**。
...
| Public Hostname | Docker 時：brain.zhe-wei.net → **http://brain_server:8000**；本機 tunnel 時 → localhost:8000 |
