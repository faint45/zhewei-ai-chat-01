...
2. **設定 .env**：在專案根目錄的 `.env` 填寫：
   ...
   ```
   **Token 須為「完整 Token」**（從 **Install connector** 或 **Reinstall** 複製的整段，通常為 `eyJ...` 開頭的長字串），**不是**隧道 ID（UUID）。取得：https://one.dash.cloudflare.com → **Networks** → **Tunnels** → 你的隧道 → **Install connector** 或 **Reinstall** → 複製 Token。
3. **啟動服務**：在專案根目錄執行：
   ```powershell
