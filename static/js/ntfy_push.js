 ...
 *
 * 用法：
 *   <script src="/static/js/ntfy_push.js"></script>
 *   <script>
 *     ZheweiPush.init({
...
(function (root, factory) {
  ...
  let _reconnectMs = DEFAULTS.reconnectBaseMs;
  ...
  function _log(...args) {
    if (_cfg.debug) console.log("[ZheweiPush]", ...args);
  }

  function _warn(...args) {
    console.warn("[ZheweiPush]", ...args);
  }
  ...
  function _authHeader() {
    if (!_cfg.user || !_cfg.pass) return "";
    return "Basic " + btoa(_cfg.user + ":" + _cfg.pass);
  }
  ...
  async function _requestPermission() {
    ...
    try {
      ...
      _warn("Permission request failed:", e);
  ...
  var ZheweiPush = {
     ...
     */
    getNtfyAppLink: function () {
      return _getNtfyAppLink();
    },
     ...
     */
    send: async function (payload, apiBase) {
      var base = (apiBase || "").replace(/\/+$/, "") || "";
      var token = localStorage.getItem("jarvis_token") || "";
      try {
        var resp = await fetch(base + "/api/ntfy/send", {
          method: "POST",
          headers: {
